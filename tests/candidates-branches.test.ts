...
describe('Candidates Branch Coverage', () => {
    ...
    describe('compareCandidates', () => {
        it('should handle fullAvailability rule', () => {
            const a: UsageCandidate = { remainingPercent: 100 } as any;
            const b: UsageCandidate = { remainingPercent: 99 } as any;
        ...
        it('should handle earliestReset rule branches', () => {
            const now = Date.now();
            const a: UsageCandidate = { resetsAt: new Date(now + 1000) } as any;
            const b: UsageCandidate = { resetsAt: new Date(now + 2000) } as any;
            const none: UsageCandidate = { resetsAt: undefined } as any;
            
            // Both defined
            expect(compareCandidates(a, b, ['earliestReset']).diff).toBeGreaterThan(0); // a is earlier (better)
    ...
    describe('selectionReason', () => {
        it('should return reasons for different rules', () => {
            const best: UsageCandidate = { remainingPercent: 100, resetsAt: new Date() } as any;
            const runner: UsageCandidate = { remainingPercent: 50, resetsAt: new Date(Date.now() + 10000) } as any;
            
            // fullAvailability
            expect(selectionReason(best, runner, ['fullAvailability'])).toContain('fullAvailability');
            
            // earliestReset
            const bestReset: UsageCandidate = { remainingPercent: 50, resetsAt: new Date(Date.now()) } as any;
            const runnerReset: UsageCandidate = { remainingPercent: 50, resetsAt: new Date(Date.now() + 10000) } as any;
            expect(selectionReason(bestReset, runnerReset, ['earliestReset'])).toContain('earlier reset');
            
            // remainingPercent
             const bestRem: UsageCandidate = { remainingPercent: 80 } as any;
             const runnerRem: UsageCandidate = { remainingPercent: 40 } as any;
             expect(selectionReason(bestRem, runnerRem, ['remainingPercent'])).toContain('higher availability');
             
             // Tied
             expect(selectionReason(bestRem, bestRem, ['remainingPercent'])).toBe('tied');
        });
        
        it('should handle missing runner up', () => {
             const best: UsageCandidate = { remainingPercent: 100 } as any;
             expect(selectionReason(best, undefined, [])).toBe('only available bucket');
        });
    });
    ...
    describe('findMappingBy (Model & Ignore)', () => {
        ...
        const mappings: MappingEntry[] = [
            // Exact Account
            { usage: { provider: 'prov', account: 'acc', window: 'win' }, model: { id: 'exact-acc' } },
            // Exact Generic
            { usage: { provider: 'prov', window: 'win' }, model: { id: 'exact-gen' } },
            // Pattern Account
            { usage: { provider: 'prov', account: 'acc', windowPattern: '^win$' }, model: { id: 'pat-acc' } },
            // Pattern Generic
            { usage: { provider: 'prov', windowPattern: '^win$' }, model: { id: 'pat-gen' } },
            // Catch-all Account
            { usage: { provider: 'prov', account: 'acc' }, model: { id: 'catch-acc' } },
            // Catch-all Generic
            { usage: { provider: 'prov' }, model: { id: 'catch-gen' } },
        ];
        ...
        it('should handle invalid regex gracefully', () => {
            const invalid = { usage: { provider: 'prov', windowPattern: '[' }, model: { id: 'fail' } };
    ...
    describe('dedupeCandidates', () => {
        it('should update existing candidate if new one has better remainingPercent', () => {
            const c1: UsageCandidate = { provider: 'p', account: 'a', windowLabel: 'w', remainingPercent: 10 } as any;
            const c2: UsageCandidate = { provider: 'p', account: 'a', windowLabel: 'w', remainingPercent: 90 } as any;
        ...
        it('should NOT update existing candidate if new one has worse remainingPercent', () => {
            const c1: UsageCandidate = { provider: 'p', account: 'a', windowLabel: 'w', remainingPercent: 90 } as any;
            const c2: UsageCandidate = { provider: 'p', account: 'a', windowLabel: 'w', remainingPercent: 10 } as any;

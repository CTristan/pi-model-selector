...
import { loadConfig, upsertMapping, updateWidgetConfig, saveConfigFile } from '../src/config.js';
...
describe('Config Loading', () => {
    ...
    it('should handle missing files and return default values', async () => {
        ...
        expect(config).not.toBeNull();
        expect(config?.priority).toEqual(['fullAvailability', 'remainingPercent', 'earliestReset']);
    });
    ...
    it('should normalize debug log paths', async () => {
        ...
        vi.mocked(fs.promises.readFile).mockResolvedValueOnce(JSON.stringify(debugConfig));
    ...
    it('should save config files and handle errors', async () => {
        ...
        vi.mocked(fs.promises.access).mockResolvedValue(undefined);
        await expect(saveConfigFile('/mock/path.json', { a: 1 })).rejects.toThrow("fail");
...
describe('Config Mutation', () => {
    it('should upsert mappings correctly', () => {
        ...
        expect(raw.mappings).toHaveLength(1);
        expect(raw.mappings[0].model.id).toBe('m2');
    });

    it('should update widget config', () => {
        const raw: any = { widget: { enabled: true } };
        updateWidgetConfig(raw, { enabled: false, showCount: 5 });
        expect(raw.widget.enabled).toBe(false);
        expect(raw.widget.showCount).toBe(5);
    });
});
